...
The boundary is enforced before parsing and rendering as well as after it. Each command stream and local state file has an 8 MiB limit. State readers accept only regular files and do not follow a final symbolic link. QML obtains the cache through the bounded collector interface and renders every displayed value as plain text, preventing external metadata from becoming an unbounded read or a rich-text/resource request.
