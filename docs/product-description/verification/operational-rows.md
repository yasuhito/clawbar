...
| MODEL-02 | P1 | demo | Registered Agent and previous Task Result remain independent ([Agents and Tasks](../foundations/operational-model.md#agents-and-tasks)). | Registered Agent with prior Failed result. | 1. Open panel.<br>2. Select Agent. | Static green registration dot and Task Failed remain distinct; no activity claim or Gateway Incident roll-up. | — |
...
| MODEL-06 | P1 | screen reader | Visually quiet healthy rows and the Registered Agent dot retain accessible meaning ([Interactions with other systems](../foundations/operational-model.md#interactions-with-other-systems)). | Healthy Node/Automation and Registered Agent. | 1. Traverse rows with accessibility tool.<br>2. Select the Agent. | Names and meanings are exposed; the Agent description says Registered Agent without claiming health or activity. | — |
...
## agents/registration-and-task-results.md

| ID | P | Device | Claim | Setup | Steps | Expected | Result |
| --- | --- | --- | --- | --- | --- | --- | --- |
| AGENT-01 | P1 | demo | Registered Agent marker does not claim health, online presence, or current activity ([Settled](../agents/registration-and-task-results.md#settled)). | Registered Agents with varied Task Results. | 1. Open panel.<br>2. Select every Agent. | Every row has the same static green dot; no Working, Waiting, Idle, or Activity detail appears. | — |
| AGENT-02 | P1 | demo | Failed Task Result does not create an Attention Item, Incident, or notification ([Interactions with other systems](../agents/registration-and-task-results.md#interactions-with-other-systems)). | Healthy Gateway; one Agent prior failure; no Automation failure. | 1. Refresh.<br>2. Inspect bar and notification history. | Failed result visible only on Agent; Gateway remains healthy and quiet. | — |
| AGENT-03 | P2 | demo | Empty Agent list omits the entire section ([The simple case](../agents/registration-and-task-results.md#the-simple-case)). | Available empty Agents with nonempty other sections. | 1. Open panel. | No `AGENTS` heading or empty message; other sections remain. | — |
| AGENT-04 | P2 | keyboard | Selection follows Agent key across Gateway-order changes ([While waiting](../agents/registration-and-task-results.md#while-waiting)). | Select Agent then publish reordered list. | 1. Select Agent.<br>2. Refresh reorder. | Same Agent stays selected at new position. | — |
| AGENT-05 | P2 | demo | Historical Agent row retains bounded registration and previous Task context without claiming current presence ([Settled](../agents/registration-and-task-results.md#settled)). | Current then unstable/stale state. | 1. Select Agent.<br>2. Transition historical. | Reduced emphasis and Last known observation; no current presence or activity claim. | — |
| AGENT-06 | P1 | screen reader | Agent registration and failed Task Result are discoverable accessibly ([Open questions and verification](../agents/registration-and-task-results.md#open-questions-and-verification)). | Registered Agents with Failed and None results. | 1. Traverse rows and detail.<br>2. Compare the rows. | Screen reader exposes Registered Agent and separate previous result without Activity semantics. | — |
