...
| SURF-05 | P2 | demo | User refresh and candidate verification expose distinct bounded feedback ([While waiting](../foundations/surface-and-actions.md#while-waiting)). | Existing healthy snapshot, then delayed setup verification. | 1. Refresh healthy state.<br>2. Verify candidate. | Refresh keeps existing rows and prefixes the summary with `Refreshing…`; the selected candidate explicitly shows `Verifying…`. | — |
...
| REF-04 | P1 | delayed demo | Existing Snapshot remains readable while interactive refresh gives immediate feedback ([Waiting begins](../bar/manual-refresh.md#waiting-begins)). | Existing healthy Snapshot; 8-second collection. | 1. Press `r`.<br>2. Observe bar and panel until completion.<br>3. Repeat with scheduled collection only. | Claw animates and summary starts `Refreshing…` for the user request; rows remain readable; scheduled collection does not add the text. | — |
| REF-05 | P1 | demo | Missing collector service preserves old data and gives bounded private-safe failure feedback ([Invoked](../bar/manual-refresh.md#invoked)). | Old Snapshot loaded; run widget without service entry. | 1. Press `r`.<br>2. Inspect panel and shell console.<br>3. Wait six seconds. | Summary says `Refresh failed · showing last known`, old rows remain, and the message returns to the normal summary after six seconds. | — |
...
| OPEN-06 | P1 | mouse/keyboard | Gateway identity and status stay visible while long content scrolls ([Edge cases](../panel/opening-and-closing.md#edge-cases)). | All-sections demo at maximum panel height. | 1. Open panel.<br>2. Scroll to the bottom.<br>3. Move Selection across off-screen rows. | OpenClaw name, Gateway status, summary, and observation time remain pinned; only rows move and the selected row stays visible below the header. | — |
...
| NAV-06 | P1 | mouse/keyboard | A thin indicator communicates list length and scroll position without covering content ([Settled](../panel/navigation-and-details.md#settled)). | Compare a short list with an all-sections list at maximum panel height. | 1. Open the short list.<br>2. Open the long list and scroll by wheel.<br>3. Move Selection beyond the viewport.<br>4. Stop interaction. | Short list has no indicator. Long list thumb size and position follow the viewport; it is clearer during movement and fades to a quiet resting opacity without overlapping text. | — |
| NAV-07 | P1 | mouse/keyboard | Operational detail reveals with shared short motion while Selection remains immediate ([Waiting begins](../panel/navigation-and-details.md#waiting-begins)). | All-sections demo with enough rows to scroll. | 1. Select Node, Agent, and Automation by pointer.<br>2. Repeat with J/K or arrows.<br>3. Move to a row near the viewport edge. | Border and fill change immediately; detail height and opacity settle smoothly; old detail collapses clipped; selected row stays visible and the scroll indicator follows. | — |
