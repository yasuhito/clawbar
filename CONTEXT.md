...
The single port through which the collector asks the OpenClaw and Tailscale CLIs its seven questions (Gateway status, node status, Fleet Nodes, Agents, Tasks, Automations, Tailscale devices) and reads each raw answer before the collection deadline. Production plugs in a bounded child-process adapter; tests plug in an in-memory answer table. Decoding stays with the collector, and Incident notifications do not pass through it.
_Avoid_: openclaw wrapper, command runner, notify-send
...
**Automation ID**:
The opaque identifier reported by the Gateway for an Automation. Clawbar retains it as non-display Operational Metadata only to preserve selection across refreshed snapshots.
