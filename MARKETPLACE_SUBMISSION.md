...
Reviewer remediation is included in the current repository HEAD: external metadata is rendered as plain text; command output, state files, and theme-color reads are capped at 8 MiB; and file readers reject non-regular files and final symbolic links. QML routes both the Snapshot and theme colors through these bounded collector interfaces.
